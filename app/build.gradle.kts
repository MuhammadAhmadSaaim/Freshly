plugins {
    alias(libs.plugins.androidApplication)
}

android {
    namespace = "com.example.freshly"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.example.freshly"
        minSdk = 24
        targetSdk = 34
        versionCode = 1
        versionName = "1.0"

        testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"
    }

    buildTypes {
        release {
            isMinifyEnabled = false
            proguardFiles(
                getDefaultProguardFile("proguard-android-optimize.txt"),
                "proguard-rules.pro"
            )
        }
    }
    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_1_8
        targetCompatibility = JavaVersion.VERSION_1_8
    }
}

dependencies {
    // Core Android dependencies
    implementation(libs.appcompat)
    implementation(libs.material)
    implementation(libs.activity)
    implementation(libs.constraintlayout)

    // Add RecyclerView for displaying lists of products
    implementation(libs.recyclerview)

    // Room dependencies for database functionality
    implementation(libs.room.runtime)
    annotationProcessor(libs.room.compiler) // For Java projects
    // or for Kotlin projects use:
    // kapt libs.room.compiler

    // Room coroutines support (optional)
    implementation(libs.room.ktx)

    // LiveData and ViewModel for managing UI-related data in a lifecycle-conscious way
    implementation(libs.lifecycle.livedata)
    implementation(libs.lifecycle.viewmodel)

    // Testing dependencies
    testImplementation(libs.junit)
    androidTestImplementation(libs.ext.junit)
    androidTestImplementation(libs.espresso.core)
}
